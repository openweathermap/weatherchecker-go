...
func CreateSources() (sources []SourceEntry) {
	...
	sources = append(sources, SourceEntry{Name: "wunderground", PrettyName: "Weather Underground", Urls: map[string]string{"current": `http://api.wunderground.com/api/{{.Source.Keys.Key}}/conditions/q/{{.Location.Latitude}},{{.Location.Longitude}}.json`,
	...
	sources = append(sources, SourceEntry{Name: "myweather2", PrettyName: "MyWeather2", Urls: map[string]string{"current": `http://www.myweather2.com/developer/forecast.ashx?uac={{.Source.Keys.Key}}&query={{.Location.Latitude}},{{.Location.Longitude}}&temp_unit=c&output=json&ws_unit=kph`,
	...
	sources = append(sources, SourceEntry{Name: "forecast.io", PrettyName: "Forecast.io", Urls: map[string]string{"current": `https://api.forecast.io/forecast/{{.Source.Keys.Key}}/{{.Location.Latitude}},{{.Location.Longitude}}?units=si`,
	...
	sources = append(sources, SourceEntry{Name: "worldweatheronline", PrettyName: "WorldWeatherOnline", Urls: map[string]string{"current": `http://api.worldweatheronline.com/free/v2/weather.ashx?key={{.Source.Keys.Key}}&q={{.Location.Latitude}},{{.Location.Longitude}}&format=json&fx=no&date_format=unix`,
