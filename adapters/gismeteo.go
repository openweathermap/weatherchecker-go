...
func normalize_pressure(pressure float64, unit string) (float64, error) {
    ...
    if u_ok == false {return float64(0), unitNotFoundError}
...
func GismeteoAdaptCurrentWeather(htmlString string) MeasurementArray {
    ...
    nodeIndex1 := 0
    if len(nodeGroup1) < nodeIndex1 + 1 {} else {node1 := nodeGroup1[nodeIndex1];temp_raw = node1.Data}
    ...
    humidity_raw, im_ok := itemMap["humidity"];if im_ok == true {humidity, err := strconv.ParseFloat(humidity_raw, 64); if err == nil {entry.Humidity = humidity}}
